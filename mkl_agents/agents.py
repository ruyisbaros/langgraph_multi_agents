...
def agent_preprocessor(state: AgentState):
    ...
    result = preprocessor_agent_generator.invoke(
...
def agent_code_generation(state: AgentState):
    ...
    if (state["generated_code_result"] == "regenerate" or
        ...
        print(colored(
            "DEBUG: Initial Generation of code. No need to reset agent state.", "green"))

    # Continue with the rest of your code generation logic...
    result = agent_code_generator.invoke(
        {"task": state["preprocessor_agent_result"]})
...
def agent_extract_code(state: AgentState):
    ...
    code_block_with_lang = re.search(
        r"```python(.*?)```", code_result, re.DOTALL)
    ...
    # 2. If that fails, try to extract from triple backticks with 'python'
        ...
        print(colored(
            "DEBUG: Extracted Python Code from triple backticks with 'python'", "green"))
    ...
    # 4. Fallback: Assume the entire result is the code if no backticks are found
        ...
        print(colored(
            "DEBUG: No backticks found. Assuming entire result is the code.", "yellow"))
        ...
        # Extraction failed, regenerate
        state["code_extraction_status"] = "regenerate"
...
def agent_code_review(state: AgentState):
    ...
    code_review_result = code_review_agent_generator.invoke(
        {"generated_code": state["extracted_python_code"], "initial_request": state["preprocessor_agent_result"]})
    ...
    try:
        ...
        print(
            colored(f"ERROR: Code review validation failed with error: {e}", "red"))
        state["code_review_status"] = "regenerate"
...
def agent_execute_code_in_docker(state: AgentState):
    ...
    try:
        container_output = client.containers.run(
            ...
            volumes={os.path.dirname(temp_code_filename): {
        ...
        print(
            colored(f"ERROR: Error running code in container: {str(e)}", "red"))
